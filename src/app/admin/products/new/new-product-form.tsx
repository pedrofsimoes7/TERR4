...
const PRODUCT_STATUSES: ProductStatus[] = ["AVAILABLE", "COMING_SOON", "DRAFT"];
...
export function NewProductForm({
  ...
  async function handleUpload(file: File, index?: number) {
    if (index !== undefined) setUploadingIdx(index);
  ...
  function setPrimary(index: number) {
    setImages((prev) => {
      ...
      return [{ ...selected, isPrimary: true }, ...others.map((img) => ({ ...img, isPrimary: false }))];
    });
  }
  ...
  return (
    <form action={createAction} className="mt-10 space-y-6">
      {/* Hidden inputs com URLs das imagens */}
      {images.map((img, index) => (
        <input key={`${img.url}-${index}`} type="hidden" name={`imageUrl_${index}`} value={img.url} />
      ))}
      <input type="hidden" name="imageCount" value={images.length} />

      {/* ── Gestor de fotos ── */}
      <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
        <h2 className="mb-5 text-lg font-black text-white">Fotos do produto</h2>

        {primaryImage ? (
          <div className="relative mb-5 overflow-hidden rounded-[1.5rem] border border-white/10 bg-black/20">
            <Image src={primaryImage.url} alt="Foto principal" width={1200} height={600} className="aspect-[16/7] w-full object-cover" />
          ...
          <div className="mb-5 flex aspect-[16/7] items-center justify-center rounded-[1.5rem] border border-dashed border-white/15 bg-black/20 text-sm text-white/30">
            Ainda sem fotos
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
          {images.map((img, index) => (
            <div
              ...
              className={`group relative overflow-hidden rounded-2xl border-2 transition-all duration-200 ${
                img.isPrimary ? "border-[#c46a2d]" : "border-white/10 hover:border-white/25"
              }`}
            >
              <div className="relative aspect-square overflow-hidden bg-black/30">
                <Image src={img.url} alt={`Foto ${index + 1}`} fill className="object-cover" />
                  ...
                  <div className="flex gap-2">
                    {index > 0 && (
                      <button type="button" onClick={() => moveImage(index, index - 1)}
                        className="flex size-7 items-center justify-center rounded-full bg-white/15 text-xs text-white hover:bg-white/30">←</button>
                    )}
              ...
              <label className="block cursor-pointer border-t border-white/8 bg-white/[0.03] px-2 py-1.5 text-center text-[10px] font-bold text-white/40 transition hover:bg-white/[0.07] hover:text-white/70">
                ...
                <input type="file" accept="image/*" className="hidden"
                  onChange={(e) => { const f = e.target.files?.[0]; if (f) handleUpload(f, index); }} />
              </label>
              {img.isPrimary && (
          ...
          <label className={`group flex aspect-square cursor-pointer flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-white/15 transition-all hover:border-white/35 hover:bg-white/[0.04] ${uploading ? "pointer-events-none opacity-50" : ""}`}>
        ...
        <p className="mt-3 text-xs text-white/25">A primeira foto é a foto principal. Usa as setas para reordenar.</p>
      </div>
      ...
      <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-7">
        <div className="grid gap-6">
          ...
          <Input name="slug" label="Slug" placeholder="ex: terr4-camp-chair" />
          ...
          <Textarea name="description" label="Descrição longa" rows={6} />
          <Textarea name="specs" label="Detalhes técnicos (label: valor, uma por linha)" rows={7}
            placeholder={"Estrutura: Metálica reforçada\nCapacidade: 120 kg"} />
          <Textarea name="included" label="Incluído (uma por linha)" rows={4}
            placeholder={"Cadeira\nSaco de transporte"} />
          <Textarea name="features" label="Features (título: descrição, uma por linha)" rows={5}
            placeholder="Dobrável e compacta: Sistema de dobragem rápido." />
          <Textarea name="trustItems" label="Badges rápidos (label: valor)" rows={3}
            defaultValue={"Envio: A combinar\nGarantia: 12 meses\nInclui: Produto TERR4"} />
          <Textarea name="compatibility" label="Compatibilidade (opcional)" rows={3}
            placeholder="Opcional. Usa só quando fizer sentido." />
          <Textarea name="warranty" label="Garantia" rows={3}
...
function Input({ label, name, defaultValue, placeholder, type = "text", step }: {
