...
export default async function GalleryPage() {
  ...
  const gallery = galleryImages.map((g) => ({
    id: g.id,
    url: g.url,
    alt: g.alt,
  }));
        ...
        <div className="mx-auto max-w-7xl">
          <Reveal>
            ...
            <p className="mt-6 max-w-xl text-lg leading-8 text-white/65">
              Momentos reais de quem leva a TERR4 para a estrada. Vê a galeria
              e lê as experiências da nossa comunidade.
            </p>
          </Reveal>
        </div>
      </section>
            ...
            {reviews.length === 0 ? (
              <div className="rounded-[2rem] border border-dashed border-white/12 bg-white/[0.02] p-12 text-center">
                ...
                <p className="mt-3 text-sm text-white/45">
                  Sê o primeiro a partilhar a tua experiência com a TERR4.
                </p>
              </div>
            ) : (
                ...
                {reviews.map((review) => (
                  <StaggerItem key={review.id}>
                    <article className="flex h-full flex-col rounded-[1.75rem] border border-white/10 bg-[#151411] p-6">
                      ...
                      <p className="mt-3 flex-1 text-sm leading-7 text-[#c8c4be]/65">
                        {review.body}
                      </p>
